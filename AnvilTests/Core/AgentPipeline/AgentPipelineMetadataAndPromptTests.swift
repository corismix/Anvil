...
extension AgentPipelineTests {
    ...
    func promptRefinementScopeMatchesCodingAgentCapacity() {
        ...
        #expect(spark.contains("Choose at most 3 core user-facing features for the first version."))
        #expect(spark.contains("Never drop a requested feature silently."))
        #expect(spark.contains("Deferred for a later version:"))
        ...
        for instructions in [flame, codex] {
            ...
            #expect(instructions.contains("Keep the refined prompt under 200 words."))
            #expect(instructions.contains("Make every implicit choice explicit"))
