...
extension InferenceTests {
    ...
    func imageGenerationProviderDefaultsToAutomaticAndPersists() {
        let suiteName = "AnvilTests.ImageGenerationProvider.\(UUID().uuidString)"
    ...
    func generatedPromptRefinementPreferenceDefaultsEnabledAndPersists() {
        let suiteName = "AnvilTests.GeneratedPromptRefinement.\(UUID().uuidString)"
        let userDefaults = UserDefaults(suiteName: suiteName)!
        userDefaults.removePersistentDomain(forName: suiteName)
    ...
    func codingAgentPreferenceDefaultsAutomaticAndPersists() {
        let suiteName = "AnvilTests.ToolCodingAgent.\(UUID().uuidString)"
        let userDefaults = UserDefaults(suiteName: suiteName)!
        userDefaults.removePersistentDomain(forName: suiteName)
        ...
        let reloadedPreferences = GenerationPreferencesStore(userDefaults: userDefaults)
        #expect(reloadedPreferences.codingAgentPreference == .anvilFlame)
    ...
    func reasoningEffortDefaultsAndPersists() {
        let suiteName = "AnvilTests.ReasoningEffort.\(UUID().uuidString)"
        let userDefaults = UserDefaults(suiteName: suiteName)!
        userDefaults.removePersistentDomain(forName: suiteName)
    ...
    func codingAgentPreferencePreservesLegacySmallAndLargeRawValues() {
        let suiteName = "AnvilTests.ToolCodingAgentLegacy.\(UUID().uuidString)"
        let userDefaults = UserDefaults(suiteName: suiteName)!
        userDefaults.removePersistentDomain(forName: suiteName)

        userDefaults.set("small_model", forKey: "generation.agentPipelineProfile")
        #expect(GenerationPreferencesStore(userDefaults: userDefaults).codingAgentPreference == .anvilSpark)

        userDefaults.set("large_model", forKey: "generation.agentPipelineProfile")
        #expect(GenerationPreferencesStore(userDefaults: userDefaults).codingAgentPreference == .anvilFlame)
    }

    @MainActor
    @Test
    func automaticGeneratedAppPermissionsDefaultOnAndPersist() {
        let suiteName = "AnvilTests.AutomaticGeneratedAppPermissions.\(UUID().uuidString)"
        let userDefaults = UserDefaults(suiteName: suiteName)!
        userDefaults.removePersistentDomain(forName: suiteName)
    ...
    func generatedAppResourcePermissionPreferencesDefaultOffAndPersist() {
        let suiteName = "AnvilTests.GeneratedAppResourcePermissions.\(UUID().uuidString)"
        let userDefaults = UserDefaults(suiteName: suiteName)!
        userDefaults.removePersistentDomain(forName: suiteName)
    ...
    func generatedAppSandboxPermissionPreferencesPreserveLegacyDefaultsAndPersist() {
        let suiteName = "AnvilTests.GeneratedAppSandboxPermissions.\(UUID().uuidString)"
        let userDefaults = UserDefaults(suiteName: suiteName)!
        userDefaults.removePersistentDomain(forName: suiteName)
    ...
    func explicitToolCodingAgentOverridesAutomaticProviderDefault() async throws {
        ...
        let context = try await store.makeSelectedAgentLanguageModelContext()
        #expect(context.pipelineConfiguration.codingAgent == .anvilFlame)
        #expect(
    ...
    func automaticCodingAgentUsesProviderModelAndLargeEditRules() {
        ...
        #expect(resolve("claude-local", provider: ollamaProvider) == .anvilFlame)
        #expect(resolve("claude-local", provider: ollamaProvider, lineCount: 601) == .codex)
        #expect(resolve("gemma4:12b", provider: ollamaProvider, lineCount: 601) == .anvilSpark)

        customProvider.openAICompatibleAPIVariant = .chatCompletions
        #expect(resolve("openai/gpt-5.4", provider: customProvider) == .anvilFlame)
        #expect(resolve("openai/gpt-5.4", provider: customProvider, lineCount: 601) == .anvilFlame)

        customProvider.openAICompatibleAPIVariant = .responses
        #expect(resolve("openai/gpt-5.4", provider: customProvider) == .codex)
        #expect(resolve("google/gemini-3.1", provider: customProvider) == .anvilFlame)
        #expect(resolve("google/gemini-3.1", provider: customProvider, lineCount: 601) == .codex)
        #expect(resolve("deepseek-v4", provider: customProvider, lineCount: 601) == .anvilSpark)

        #expect(resolve("claude-sonnet", provider: anthropicProvider, lineCount: 601) == .anvilFlame)
        #expect(
            resolve(
                "openai/gpt-5.4",
                provider: openAIProvider,
                requested: .anvilSpark
            ) == .anvilSpark
        )
    }
    ...
    func selectedOllamaOpenAIModelUsesUnauthenticatedResponsesProviderForCodex() async throws {
        ...
        #expect(codexProvider.configurationIdentifier == "anvil_ollama")
