...
private enum ToolLibraryGenerationError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .missingPreparedTool:
            return "Anvil could not finish this app because generation did not prepare a library record."
        }
    }
}
...
final class ToolLibraryStore {
    ...
    private func resumeGeneration(
        ...
        guard !resumePrompt.isEmpty else {
            presentError("Anvil does not have enough saved prompt context to continue this generation.")
            return
        }
