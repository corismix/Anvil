...
Snapshot of the codebase as inherited from Anvil at a0e29c1, updated as
...
- `Anvil/App` - startup, `AnvilApplicationController` (wires
  persistence, stores, routing, windows), URL routing, menu.
- `Anvil/Core/Models` - domain enums and typealiases to the current
  persistence schema.
- `Anvil/Core/Persistence` - SwiftData container, versioned schemas
  (`AnvilSchemaV1..V7`, one migration plan), repositories, app paths
  (`AnvilPaths`, data under `~/.anvil/`), preferences.
- `Anvil/Core/Inference` - provider catalog, credentials (Keychain),
...
- `Anvil/Core/AgentPipeline` - the harness: generated-package layout
...
- `Anvil/Features` - `Launch` (Command Line Tools gate, onboarding),
...
- Generated tools are SwiftPM packages under `AnvilPaths.toolsDirectory`;
...
- Supabase auth (Apple OAuth, email/password) via `AnvilAccountClient`.
- Credits: balance, ledger, credit packs, Stripe checkout sessions via the
  upstream API (`api/v1/...`).
- `Anvil` inference provider: OpenAI-compatible proxy on the upstream
...
- Backend config via `Config/.env` / Info.plist keys
  (`AnvilSupabaseURL`, `AnvilSupabasePublishableKey`,
  `AnvilAPIBaseURL`).
